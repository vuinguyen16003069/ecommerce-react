    ...
    <div className="group bg-white rounded-2xl border border-gray-100/80 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden flex flex-col h-full">
      ...
      {product.stock <= 0 && (
        <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-20 flex items-center justify-center">
          <div className="bg-white/90 px-4 py-2 rounded-xl border border-gray-200 shadow-sm shadow-black/5 transform -rotate-12 italic text-gray-800 font-bold whitespace-nowrap">
            Hết hàng
          </div>
        </div>
      )}
      ...
      <Link
        ...
        className="relative h-48 bg-gray-50 flex items-center justify-center overflow-hidden"
      >
        {product.isFlashSale && (
          <div className="absolute top-2 left-2 z-10 bg-red-500 text-white px-3 py-1 rounded-lg text-[10px] font-black">
        ...
        <img
          src={product.image}
          alt={product.name}
          className="max-h-[85%] object-contain group-hover:scale-110 transition-transform duration-500 ease-out"
        />
        <button
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="text-xs text-orange-600 font-bold mb-1.5 uppercase tracking-wide">
          {product.category}
        </div>
        <Link
          to={`/product/${product._id || product.id}`}
          className="font-bold text-gray-800 mb-1 line-clamp-2 hover:text-orange-600 transition min-h-[40px] leading-tight"
        >
          {product.name}
        </Link>
        ...
        <div className="mt-auto flex items-end justify-between">
          {product.isFlashSale ? (
            <div className="flex items-end gap-2">
              <div className="font-black text-lg text-orange-600 tracking-tight">
