...
const CATEGORIES = ['Tất cả', 'Thời trang nam', 'Thời trang nữ', 'Giày dép', 'Phụ kiện', 'Công nghệ']
...
const ProductsPage = () => {
  ...
  const [form, setForm] = useState({
    name: '', price: '', category: 'Thời trang nam', stock: '', image: '', desc: '',
    isFlashSale: false, flashSaleDiscount: 50, flashSaleStartTime: '', flashSaleEndTime: ''
  })
  ...
  const openAdd = () => {
    setEditing(null)
    setForm({ name: '', price: '', category: 'Thời trang nam', stock: '', image: 'https://via.placeholder.com/400', desc: '', isFlashSale: false, flashSaleDiscount: 50, flashSaleStartTime: '', flashSaleEndTime: '' })
    setModal(true)
  }
  const openEdit = (p) => {
    setEditing(p)
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      ...
      <div className="flex flex-wrap justify-between items-center gap-3 mb-5">
        <h3 className="font-bold text-gray-900">Danh sách sản phẩm ({displayed.length}/{products.length})</h3>
              ...
              <tr key={p._id || p.id} className="hover:bg-gray-50 transition">
                <td className="p-3">
                  <div className="flex items-center gap-3">
                    <img src={p.image} className="w-9 h-9 rounded-lg object-cover border flex-shrink-0" alt="" />
                ...
                <td className="p-3">
                  <span className={`px-2 py-0.5 rounded-md text-xs font-bold ${p.stock > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>{p.stock}</span>
                </td>
                <td className="p-3 text-gray-500 text-xs font-medium">{p.sold || 0}</td>
                <td className="p-3 text-gray-500 text-xs">{p.category}</td>
                <td className="p-3">
                  {p.isFlashSale
                    ? <span className="text-[10px] bg-red-100 text-red-600 font-bold px-2 py-0.5 rounded-full">-{p.flashSaleDiscount}%</span>
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title={editing ? 'Cập nhật sản phẩm' : 'Thêm sản phẩm mới'}>
        <form onSubmit={submit} className="space-y-4">
          <div>
            ...
            <input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 transition" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-gray-600 block mb-1">Giá (VND)</label>
              <input required type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 transition" />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-600 block mb-1">Tồn kho</label>
              <input required type="number" value={form.stock} onChange={(e) => setForm({ ...form, stock: e.target.value })} className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 transition" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <input value={form.image} onChange={(e) => setForm({ ...form, image: e.target.value })} className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 transition" />
          </div>
          <div>
            <label className="text-xs font-bold text-gray-600 block mb-1">Mô tả</label>
            <textarea rows="3" value={form.desc} onChange={(e) => setForm({ ...form, desc: e.target.value })} className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 resize-none transition"></textarea>
          </div>
          ...
          <div className="border-t pt-4 mt-4">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <label htmlFor="isFlashSale" className="text-sm text-gray-700 font-medium cursor-pointer">Bật Flash Sale</label>
            </div>
            {form.isFlashSale && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="datetime-local" value={form.flashSaleStartTime} onChange={(e) => setForm({ ...form, flashSaleStartTime: e.target.value })} className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 transition" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-600 block mb-1">Kết thúc</label>
                    <input type="datetime-local" value={form.flashSaleEndTime} onChange={(e) => setForm({ ...form, flashSaleEndTime: e.target.value })} className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-orange-200 focus:border-orange-400 transition" />
                  </div>
                </div>
              </div>
            )}
