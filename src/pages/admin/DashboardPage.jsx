...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    revenueChart.current = new Chart(revenueRef.current, {
      ...
      options: {
        ...
        scales: {
          y: { grid: { color: '#f1f5f9' }, ticks: { callback: (v) => formatPrice(v) } },
  ...
  return (
    <div className="space-y-6">
      <div className="mb-6">
        <h2 className="text-xl font-black text-gray-900">Tổng quan</h2>
      </div>
            ...
            <tbody className="divide-y divide-gray-50">
              {recentOrders.map((o) => (
                <tr key={o._id} className="hover:bg-gray-50 transition">
                  <td className="p-3 font-bold text-orange-600 text-xs">
                    #{o.orderId || o._id?.slice(-6)}
                  </td>
                  <td className="p-3 text-gray-700 text-xs font-medium truncate max-w-[100px]">
                    {o.customerName}
                  </td>
                  <td className="p-3 text-gray-800 text-xs font-bold">{formatPrice(o.total)}</td>
                  <td className="p-3">
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${STATUS_COLORS[o.status] || 'bg-gray-100 text-gray-600'}`}
                    >
                      {o.status}
                    </span>
                  </td>
