    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {roles.map((r) => (
          <div
            key={r._id || r.name}
            className="border border-gray-200 rounded-xl p-4 hover:border-orange-200 transition bg-gray-50/50"
          >
            ...
            <div className="mt-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {(r.permissions || []).map((slug) => (
                  <span
                    key={slug}
                    className="text-xs bg-white border px-2 py-0.5 rounded text-gray-700 truncate max-w-full"
                  >
                    {slug}
                  </span>
                ))}
      ...
      <Modal
        ...
        title={editing ? `Sửa vai trò: ${editing.name}` : 'Tạo Vai trò mới'}
      >
          ...
          <div className="border-t pt-3 mt-2">
            ...
            {allPerms.length === 0 ? (
              <p className="text-xs text-gray-400 italic">
                Chưa có quyền nào. Tạo quyền trước tại trang Quyền hạn.
              </p>
            ) : (
                ...
                {allPerms.map((p) => (
                  ...
                  >
                    ...
                    <div className="overflow-hidden">
                      <p
                        className={`text-xs font-semibold truncate ${form.permissions.includes(p.slug) ? 'text-orange-800' : 'text-gray-700'}`}
                      >
                        {p.name}
                      </p>
