...
const RegisterPage = () => {
  ...
  if (step === 3) {
    return (
      <AuthLayout title="Thành công!" subtitle="Bạn đã đăng ký tài khoản thành công." image="https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=1200&q=80">
        <div className="bg-green-50 border border-green-200 p-6 rounded-xl flex flex-col items-center text-center mb-8">
          ...
          <p className="text-green-700 text-sm mb-4">Bạn đã xác nhận email và có thể bắt đầu mua sắm ngay.</p>
        </div>
        <button onClick={() => navigate('/')} className="w-full bg-green-600 text-white font-black py-4.5 rounded-2xl shadow-lg hover:bg-green-700 transition-all uppercase tracking-widest text-sm flex items-center justify-center gap-3 group cursor-pointer">
          TIẾP TỤC SHOPPING <ArrowRight size={18} className="group-hover:translate-x-1.5 transition-transform" />
        </button>
      </AuthLayout>
    )
  }
  ...
  if (step === 2) {
    return (
      <AuthLayout title="Xác nhận OTP" subtitle="Nhập mã OTP 6 số từ email của bạn." image="https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=1200&q=80">
        ...
        <form onSubmit={handleVerifyOtp} className="space-y-6">
          ...
          >
            {loading ? (
              ...
              <>XÁC NHẬN <ArrowRight size={18} className="group-hover:translate-x-1.5 transition-transform" /></>
            )}
          </button>
        </form>

        <div className="mt-8 text-center">
          <button type="button" onClick={handleResendOtp} disabled={loading} className="text-gray-500 font-medium hover:text-orange-600 transition-colors cursor-pointer disabled:opacity-50">
  ...
  return (
    <AuthLayout title="Tạo tài khoản" subtitle="Trở thành thành viên để nhận ngay ưu đãi độc quyền." image="https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=1200&q=80">
      ...
      <form onSubmit={handleRegister} className="space-y-5">
        ...
        <button disabled={loading} type="submit" className="w-full bg-orange-600 text-white font-black py-4.5 rounded-2xl shadow-xl shadow-orange-600/20 hover:bg-orange-700 transition-all uppercase tracking-widest text-sm flex items-center justify-center gap-3 group mt-6 cursor-pointer">
