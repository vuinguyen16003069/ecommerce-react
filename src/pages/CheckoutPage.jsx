...
const CheckoutPage = () => {
  ...
  const selectedFromState = location.state?.items;
  const checkoutItems = (
    selectedFromState && selectedFromState.length ? selectedFromState : cart
  ).filter(Boolean);
  ...
  const shipping =
    checkoutItems.length === 0 ? 0 : cartTotal > 299000 ? 0 : 30000;
  ...
  }, [currentUser, checkoutItems.length, navigate]);
    ...
    try {
      ...
      if (selectedFromState && selectedFromState.length) {
        const idsToRemove = new Set(
          selectedFromState.map((item) => item._id || item.id),
        );
        const isFullCheckout = cart.every((item) =>
          idsToRemove.has(item._id || item.id),
        );
        if (isFullCheckout) {
          clearCart();
        } else {
    ...
    <div className="bg-gray-50 min-h-screen pt-12 pb-24 border-t border-gray-100">
      <div className="container mx-auto px-4 max-w-6xl animate-fade-in">
        <h2 className="text-3xl font-black text-gray-900 mb-8 tracking-tight flex items-center gap-3">
          <div className="w-1.5 h-8 bg-orange-600 rounded-full"></div> Thanh
          toán an toàn
        </h2>
          ...
          <div className="bg-orange-50/50 border border-orange-100 p-6 rounded-2xl mb-8 flex items-start gap-4 shadow-sm">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mb-3">
                Đăng nhập ngay để theo dõi lịch sử đơn hàng và nhận các ưu đãi
                hấp dẫn dành riêng cho thành viên.
              </p>
          ...
          <div className="w-full lg:w-[420px]">
            <div className="bg-gray-900 text-white p-8 rounded-3xl shadow-2xl relative overflow-hidden">
              ...
              <h3 className="font-bold text-xl mb-6 flex items-center gap-2">
                <CheckCircle size={20} className="text-green-400" /> Tóm tắt đơn
                hàng
              </h3>
                ...
                {checkoutItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-sm truncate pr-2 text-gray-100">
                        {item.name}
                      </p>
