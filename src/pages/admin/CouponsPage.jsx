...
const CouponsPage = () => {
  ...
  const openAdd = () => {
    setEditing(null);
    setForm({ code: '', discount: '', desc: '' });
    setModal(true);
  };
  const openEdit = (c) => {
    setEditing(c);
    setForm({ code: c.code, discount: c.discount, desc: c.desc || '' });
    setModal(true);
  };
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      ...
      {coupons.length === 0 ? (
        <div className="py-20 text-center">
          ...
          <button
            onClick={openAdd}
            className="mt-4 text-orange-600 text-sm font-bold hover:underline cursor-pointer"
          >
            + Tạo mã đầu tiên
          </button>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {coupons.map((c) => (
            <div
              key={c._id}
              className="border border-gray-200 rounded-xl p-4 hover:border-orange-200 transition group relative bg-gray-50/50"
            >
