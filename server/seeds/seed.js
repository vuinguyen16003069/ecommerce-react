...
const mockUsers = [
  { name: 'Admin User', email: 'admin@jshop.com', password: '123', role: 'admin', status: 'active', phone: '0901234567', wishlist: [] },
  { name: 'Khách Hàng Mẫu', email: 'khach@jshop.com', password: '123', role: 'user', status: 'active', phone: '0912345678', wishlist: [] },
  { name: 'Khách Hàng Bị Khóa', email: 'locked@jshop.com', password: '123', role: 'user', status: 'locked', phone: '0988888888', wishlist: [] },
];

const mockPosts = [
  { title: 'Xu hướng thời trang Hè 2024', author: 'Admin User', date: '2024-05-01', content: 'Năm nay, các gam màu pastel nhẹ nhàng sẽ lên ngôi. Chất liệu linen thoáng mát là lựa chọn hàng đầu cho những ngày nắng nóng...', image: 'https://images.unsplash.com/photo-1469334031218-e382a71b716b?auto=format&fit=crop&q=80&w=500' },
  { title: 'Cách bảo quản giày Sneaker bền đẹp', author: 'Admin User', date: '2024-06-15', content: 'Để giày luôn như mới, bạn nên vệ sinh định kỳ 2 tuần/lần. Tránh phơi trực tiếp dưới ánh nắng mặt trời...', image: 'https://images.unsplash.com/photo-1556906781-9a412961c28c?auto=format&fit=crop&q=80&w=500' },
];
...
async function seedDatabase() {
  try {
    ...
    console.log(`✓ ${mockProducts.length} sản phẩm đã thêm`);

    await Permission.insertMany(mockPermissions);
    console.log(`✓ ${mockPermissions.length} quyền đã thêm`);

    await Role.insertMany(mockRoles);
    console.log(`✓ ${mockRoles.length} vai trò đã thêm`);

    await User.insertMany(mockUsers);
    console.log(`✓ ${mockUsers.length} người dùng đã thêm`);

    await Post.insertMany(mockPosts);
    console.log(`✓ ${mockPosts.length} bài viết đã thêm`);

    await Coupon.insertMany(mockCoupons);
    console.log(`✓ ${mockCoupons.length} mã giảm giá đã thêm`);
