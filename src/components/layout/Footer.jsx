      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
        <div>
          <div className="font-black text-2xl text-white mb-3">
            J<span className="text-orange-500">SHOP</span>
          </div>
          <p className="text-sm leading-relaxed text-slate-500">
            Hệ thống thời trang uy tín hàng đầu Việt Nam. Chất lượng — Phong cách — Đẳng cấp.
          </p>
        </div>
        <div>
          ...
          <div className="space-y-2 text-sm">
            <p className="flex items-center gap-2">
              <MapPin size={14} className="text-orange-500" /> 90 Lưu Quý Kỳ,Hòa Cường Nam,Đà Nẵng
            </p>
            <p className="flex items-center gap-2">
              <Mail size={14} className="text-orange-500" /> vuinguyen16003069@gmail.com
            </p>
          </div>
        </div>
        <div>
          ...
          <div className="flex gap-3">
            {['Facebook', 'Instagram', 'TikTok'].map((s) => (
              <button
                key={s}
                className="text-xs bg-slate-800 hover:bg-orange-600 hover:text-white text-slate-400 px-3 py-2 rounded-lg transition font-medium cursor-pointer"
              >
                {s}
              </button>
            ))}
