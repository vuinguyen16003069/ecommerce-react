  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
              ...
              <tr key={p._id || p.id} className="hover:bg-gray-50 transition">
                ...
                <td className="p-3">
                  <div className="flex items-center gap-3">
                    <img src={p.image} className="w-12 h-8 rounded object-cover border" alt="" />
      ...
      <Modal
        ...
        title={editing ? 'Sửa bài viết' : 'Bài viết mới'}
      >
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs font-bold text-gray-600 block mb-1">Link ảnh bìa</label>
              <input
                required
                value={form.image}
          ...
          <div>
            <label className="text-xs font-bold text-gray-600 block mb-1">Tóm tắt (Excerpt)</label>
            <textarea
              required
              rows="2"
              value={form.excerpt}
              onChange={(e) => setForm({ ...form, excerpt: e.target.value })}
              className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-orange-200 resize-none transition"
            ></textarea>
          </div>
          <div>
            <label className="text-xs font-bold text-gray-600 block mb-1">Nội dung chính</label>
            <textarea
              required
              rows="5"
              value={form.content}
          ...
          >
            {editing ? 'Ghi đè thay đổi' : 'Đăng bài vào DB'}
          </button>
        </form>
      </Modal>
    </div>
